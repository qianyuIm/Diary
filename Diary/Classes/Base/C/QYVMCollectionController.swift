//
//  QYVMCollectionController.swift
//  Diary
//
...
class QYVMCollectionController<RVM: QYRefreshViewModel>: QYCollectionController {
    /// 不使用该对象时，不会被初始化
    ...
    func bindRefreshHeader() {
        guard let refreshHeader = collectionView.refreshHeader else {
            return
        }
        // 将刷新事件传递给 refreshVM
    ...
    func bindRefreshFooter() {
        guard let refreshFooter = collectionView.refreshFooter else {
            return
        }
        // 将刷新事件传递给 refreshVM
