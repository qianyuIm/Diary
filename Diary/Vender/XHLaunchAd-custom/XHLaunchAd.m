...
-(void)setupLaunchAdEnterForeground{
    NSTimeInterval lastAppearTimeStamp = [[NSUserDefaults standardUserDefaults] doubleForKey:XHTimeForEnterForegroundStringKey];
    NSTimeInterval currentTimeStamp = [self getSystemTimestamp];
    BOOL canShow = (currentTimeStamp - lastAppearTimeStamp) > _imageAdConfiguration.timeForEnterForeground;
    if (!canShow) {
        return;
    }
...
-(void)removeOnly{
    [[NSUserDefaults standardUserDefaults] setDouble:[self getSystemTimestamp] forKey:XHTimeForEnterForegroundStringKey];
    DISPATCH_SOURCE_CANCEL_SAFE(_waitDataTimer)
    DISPATCH_SOURCE_CANCEL_SAFE(_skipTimer)
...
/// 获取系统时间戳
- (NSTimeInterval)getSystemTimestamp {
